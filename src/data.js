...
const rawTweets = userArr.map((user, idx) => {
  ...
  const timeOffset = new Date(dataArr[dataArr.length - 1].created_at).getTime()
...
export const solveSameTimeTweets = (tweets) => {
  const minute = 1000
  let startTimes = {}
...
const tweets = solveSameTimeTweets(rawTweets)
